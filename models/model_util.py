...
def tf_gather_object_pc(point_cloud, mask, npoints=512):
    ...
    Output:
        object_pc: TF tensor in shape (B,npoint,C) [32*512*4]整体
    ...
    def mask_to_indices(mask):
        ...
        for i in range(mask.shape[0]):
            ...
            # 文件序号
        return indices
        # (32, 512, [batch内的文件序号，乱序的点位置])

    # tf.py_func()接收的是tensor，然后将其转化为numpy array送入我们自定义的函数，
    # 最后再将my_func函数输出的numpy array转化为tensor返回。
    indices = tf.py_func(mask_to_indices, [mask], tf.int32)
    #       2048个0或1  ->   512个[,]，取出物体
    # (32, 2048)->(32, 512, [batch内的文件序号，乱序的点位置])
...
def point_cloud_masking(point_cloud, logits, end_points, xyz_only=True):
    ...
    Output:
        object_point_cloud: TF tensor in shape (B,M,3) for simplicity we only keep XYZ here
    ...
    # sum(32, 2048, [1,1,1])*(32, 2048, [x,y,z])->(32, 1, [总x,总y,总z])
    # 对应元素相乘，算出mask点的坐标之和
    ...
    # (32, 1, [x_mean, y_mean, z_mean])
    # mean = (32, 1, [总x,总y,总z])/maximum((32, 1, [mask点个数,mask点个数,mask点个数]), 1),
    # max防止为0，即不包含mask点时
    # mask点坐标之和/mask点的数量
    ...
    # (32, 2048, [x, y, z]) - (32, 2048, [x_mean, y_mean, z_mean])
    # 算出每个点坐标到mask平均坐标的偏移量
    # mask_coordinate
