...
import sys
import argparse
import importlib # 导入模型
import numpy as np
import tensorflow as tf
from datetime import datetime
BASE_DIR = os.path.dirname(os.path.abspath(__file__)) # train
ROOT_DIR = os.path.dirname(BASE_DIR) # frustum-pointnets-master
...
MODEL = importlib.import_module(FLAGS.model) # import network modulepi
MODEL_FILE = os.path.join(ROOT_DIR, 'models', FLAGS.model+'.py')
LOG_DIR = FLAGS.log_dir
if not os.path.exists(LOG_DIR): os.mkdir(LOG_DIR)
os.system('copy %s %s' % (MODEL_FILE, LOG_DIR)) # bkp of model def
os.system('copy %s %s' % (os.path.join(BASE_DIR, 'train.py'), LOG_DIR))
...
def train():
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            #tf.get_variable(name,shape=[3, 4],dtype=None,initializer=None,)
...
if __name__ == "__main__":
    log_string('pid: %s'%(str(os.getpid()))) # 获取进程
    train()
    LOG_FOUT.close()
